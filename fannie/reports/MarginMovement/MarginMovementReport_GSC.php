...
class MarginMovementReport_GSC extends FannieReportPage 
{
    public $description = '[George Street Margin Movement] lists item movement with margin information.';
    public $report_set = 'Movement Reports';

    protected $title = "Fannie : George Street Margin Movement Report";
    protected $header = "George Street Margin Movement Report";

    protected $sort_column = 5;
    protected $sort_direction = 1;

    protected $report_headers = array('UPC', 'Brand', 'Desc', 'Dept#', 'Dept', '# Sold', '$ Sold', '$ Cost', '$ Profit', '% Margin', '$ Item Markup', '% Profit Contrib');
    ...
    {
        $profit = $row['total'] == 0 ? 0 : ($row['total'] - $row['cost']);
        ...
        return array(
            ...
            $row['department'],
            $row['dept_name'],
            $row['scale']? sprintf($row['qttyEnforced']? '%.1f' : '%.2f', $row['qty']) : $row['qty'],
    ...
    public function calculate_footers($data)
    {
        ...
        return array('Totals', null, null, null, null, null, sprintf('%.2f',$sum_ttl), sprintf('%.2f',$sum_cost), printf('%.2f',$sum_ttl-$sum_cost), '', null, null);
    }
